    ...
    try:
        ...
        for column in cleaned_data.columns:
            try:
                ...
                if isinstance(col_data, pd.Series) :  # Ensure we are working with a Series
                    if pd.api.types.is_datetime64_any_dtype(col_data):
                        # Convert datetime columns to ISO format strings
                        cleaned_data[column] = col_data.apply(
                            lambda x: x.isoformat() if pd.notnull(x) else None
                        )
                    elif isinstance(cleaned_data['SourceTracking'], pd.DataFrame):
                        cleaned_data['SourceTracking'] = cleaned_data['SourceTracking'].apply(lambda x: x[0] if isinstance(x, list) else x)
                    ...
                    print(f"Column '{column}' is not a Series and will be skipped.")
