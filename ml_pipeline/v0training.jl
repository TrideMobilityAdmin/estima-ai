...
# ╔═╡ 015946e2-7eb1-4106-ad56-3440ce56068d
begin
	using CSV
	using DataFrames
	using Transformers
...
# ╔═╡ 004e7aca-ee1e-11ef-28be-61708ee0b4b8
...
# ╔═╡ 0930d157-a49c-43d8-b1e0-96d383c39491
...
# ╔═╡ a6d8d4ab-d7e3-4ac4-943d-b347e7b277d4
function create_partval_emb(col, pricing, part_index)
...
# ╔═╡ 462baf6f-9138-4e9f-a8a4-39ea94a7da24
function split_and_trim(s)
    return first(split(strip(something(s, "")), "\n"))
end

# ╔═╡ 9db1350b-2c23-4471-8965-5ed6e589984d
...
# ╔═╡ bd2d5a65-4c3b-44dc-8883-204184bb1e52
function cosine_similarity(vec1, vec2)
    return dot(vec1, vec2) / (norm(vec1) * norm(vec2))
end

# ╔═╡ f465ea34-f6e4-40e8-83a3-31fa4bac6888
...
# ╔═╡ f8f27019-56c4-4ac2-b23d-44a9e252d5dd
...
# ╔═╡ 306f4642-2f72-4991-aaa8-fc057c64e02e
function create_the_co_occurrence_matrix(mldp, mltask, task_clusters, disc_clusters)
...
# ╔═╡ cc8b0101-4457-4e24-bc90-16cbd4661ec3


# ╔═╡ 29c8fb0f-beaf-4694-8860-d5cf4b4a10bf
function parts_prep(pricing, mldp, mltask)
	...
	mldp.partval = create_partval_emb(mldp."discrep.-#", pricing, part_index)
	mltask.partval = create_partval_emb(mltask."task-#", pricing, part_index)
	return parts, part_index, part_price, mldp, mltask
end

# ╔═╡ 85ebc723-1116-4b5c-8343-869fb7c4bcc1
function train(mldpv, mltaskv)
	mltaskv.embs, training_outcomes = train_embeddings(mltaskv.description)
	task_clusters=incremental_clustering(mltaskv)
	disc_clusters=dbscan_clustering_while_mutating_df(mldpv)
	coom=create_the_co_occurrence_matrix(mldpv,mltaskv, task_clusters, disc_clusters)
	return task_clusters, disc_clusters, coom, training_outcomes
end

# ╔═╡ 04569712-4097-4739-b70a-c690548e4f8f
begin
...
# ╔═╡ a971c2c3-90f5-4310-a0e4-b2646c96d54d
parts, part_index, part_price, mldpv, mltaskv = parts_prep(pricing, mldp, mltask)

# ╔═╡ a5af8e9a-f9f2-40a8-9ce7-d1cc333d43bc
taskc, discc, coom, train_outs = train(mldpv, mltaskv)

# ╔═╡ c4dd796f-7e60-4511-8a4d-b3c98ef20e70
begin
	CSV.write("./parts.csv", parts)
	jldsave("./trainings/part_price.jld2"; part_price)
	jldsave("./trainings/disc_clusters.jld2"; discc)
	jldsave("./trainings/task_clusters.jld2"; taskc)
	jldsave("./trainings/coom.jld2"; coom)
	jldsave("./trainings/lexicon.jld2";train_outs.lexicon)
	jldsave("./trainings/idf.jld2";train_outs.idf)
	jldsave("./trainings/training_embeddings.jld2";train_outs.training_embeddings)
end
...
manifest_format = "2.0"
project_hash = "a4a727bd4ea20f0249037899311d27e98d092016"
...
deps = ["Adapt", "ChainRulesCore", "FuncTransforms", "GPUArraysCore", "LinearAlgebra", "NNlib", "Random", "Static"]
git-tree-sha1 = "a6f8a14af3e6861634f75337c2a29daa3cf518a5"
...
[[deps.SafeTensors]]
deps = ["BFloat16s", "DLFP8Types", "JSON3", "MappedArrays", "Mmap"]
git-tree-sha1 = "b88c17966026301a2c5d798dc95d73a88d09cb74"
